...
def transform_grid(ds, data, new_grid, grid_data, dim = ['Z', 'pressure']):
    # First create an xgcm grid object
    grid = Grid(ds, coords={dim[0]: {'center':dim[1]}}, periodic=False)
    # transform to new grid
    transformed = grid.transform(data, dim[0], new_grid, target_data=grid_data)
    #.rename({'salinity':'potential_density'})
    return transformed
...
def dens_to_pres(dataArray, press_grid, P_on_d = None):
...
def even_dist_grid(dataArray, dist_interval):
    ...
    new_grid = np.arange(dataArray.distance[0], dataArray.distance[-1], dist_interval)
    ...
    if len(dataArray.dims) > 1:
        ...
        shp = (len(new_grid),)
        new_da = np.nan*np.ma.masked_all(shp)
        new_da = interpolate.interp1d(dataArray.distance.values, dataArray.values, bounds_error=False)(new_grid)
        new_da = xr.DataArray(data = new_da, dims=[dataArray.dims[0]], coords = dict(distance = ("distance", new_grid)),)
...
def gaussianFilter(DataArray, ind = None, window = 9, order = 3, interp_na = True):
    ...
    if interp_na == True:
        data = DataArray.interpolate_na(dim = DataArray.dims[0], use_coordinate = False)
    else:
        data = DataArray
    ...
    if time_only == True:
        ...
        try:
            ...
            if zdim is not None:
                ...
                new_da = xr.DataArray(np.asarray(lst), dims=float_num.hpid.dims[0])
...
def regular_time_grid(data, original_time_da, new_timestep):
    ...
    for i in range(0, len(original_time_da)):
        if ~np.isnat(original_time_da.values[i]):
            my_dict[pd.Timestamp(original_time_da.values[i]).to_pydatetime()] = data.values[i]
