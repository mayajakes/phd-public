...
def interpOnDens(float_num, floatid, dens_interval = 0.01, pdens = None, rs = False, by_dist = True):
    ...
    for var in list(float_num.data_vars):
        ...
        new_flt[var] = ((dim0, "potential_density"), new_var, float_num[var].attrs)
...
def interp_nats(data, time_grid):
    ...
    if len(data.shape) > 1:
        new_da = xr.DataArray(data.data, dims = ['time', data.dims[1]], coords = dict(time = ('time', t_interp.data)))
        new_da = new_da.assign_coords(data[data.dims[1]].coords)
    else:
        new_da = xr.DataArray(t_interp.data, dims = ['time'], coords = dict(time = ('time', t_interp.data)))    
...
def even_time_grid_2d(data, old_time_grid, new_timestep, zdim = 'pressure'):
    ...
    for p in data[zdim]:
        if zdim == 'pressure':
            d[:, i] = regular_time_grid(new_da.sel(pressure = p), old_time_grid, new_timestep)
        elif zdim =='potential_density':
            d[:, i] = regular_time_grid(new_da.sel(potential_density = p), old_time_grid, new_timestep)
        elif zdim == 'depth_cell':
            d[:, i] = regular_time_grid(new_da.sel(depth_cell = p), old_time_grid, new_timestep)
        i += 1
